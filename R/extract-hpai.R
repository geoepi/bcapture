...
#' }
#' @export
extract_hpai <- function(in_dir, out_dir, recursive = FALSE, overwrite = FALSE, quiet = FALSE) {
  ...
  hashes <- unique(stats::na.omit(manifest$form_schema_hash[manifest$status == "success"]))
  if (length(hashes) > 1L) warning("Multiple PDF form schemas were detected across the input files. Review extraction_manifest.csv before combining or analyzing fields.", call. = FALSE)
  invisible(manifest)
}
...
.extract_hpai_file_impl <- function(pdf_file, out_dir, source_relpath, overwrite, quiet) {
  ...
  tryCatch({
    ...
    metadata_values <- c(list(audit_id = audit_id, source_file = source_file, source_relpath = as.character(source_relpath), source_md5 = source_md5, number_of_pages = as.integer(parsed$number_of_pages), number_of_fields = nrow(fields), number_of_populated_fields = nrow(populated), form_schema_hash = as_optional_character(parsed$form_schema_hash), extraction_method = "acroform", pypdf_version = hpai_python_version(module), extracted_at_utc = extracted_at), parsed$pdf_metadata %||% list())
    ...
    list(audit_id = audit_id, status = "success", fields = fields, populated_fields = populated, wide = wide, metadata = metadata, output_dir = paths$dir, manifest = as_single_row_tibble(manifest))
    ...
    list(audit_id = audit_id, status = "failed", fields = NULL, populated_fields = NULL, wide = NULL, metadata = NULL, output_dir = NA_character_, error = manifest$error, manifest = as_single_row_tibble(manifest))
  })
}
